...
(Andrew Jacobs' Obelisk 6502 guide, mirrored at nesdev.org)

Complete reference for all 56 official 6502 instructions: operation, flags
affected, addressing modes, opcodes, byte counts, and cycle counts.
...
NB: The state of the decimal flag is uncertain when the CPU is powered up
...
NB: An original 6502 does not correctly fetch the target address if the
indirect vector falls on a page boundary (e.g. $xxFF where xx is any value
from $00 to $FF). In this case it fetches the LSB from $xxFF as expected
